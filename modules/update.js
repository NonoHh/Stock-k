...
function get_lastday_char(date){
    if (date[9] != '0'){
        return date.substring(0,9) + (date[9] - 1);
    }else {
        return date.substring(0,8) + (date[8] - 1) + '9';
    }
}
...
function get_lastday(stock, date) {
    var sql = 'SELECT * FROM `' + stock + '`' + ' WHERE date = \'' + get_lastday_char(date) + '\'';
