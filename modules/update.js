...
function get_close_price(stock, date, last) {
    var sql = 'SELECT * FROM `' + stock + '_rt` WHERE time = (SELECT max(time) FROM `' + stock + '_rt`)';
...
function update_close_db(stock, date, price, result) {
    ...
    if (result != false) {
        var newchange_percent = (price - result[0].close_price) / result[0].close_price * 100;
...
function insert_rt(stock, date, time, price, volume, last) {
    ...
    ;
    if (last != false) {
        if (last[0].price != 0) newchange_percent = (price - last[0].price) / last[0].price;
    }
...
function update_rt(stock, date, time, price, volume, result, last) {
    ...
    updateSql += 'price = ' + price + ', amount = ' + (volume / 100 + result[0].amount);
    if (last != false) {
        if (last[0].price != 0) updateSql += ', change_percent = ' + (price - last[0].price) / last[0].price;
    }
...
function update2(stock, date, time, price, volume, last) {
    ...
    var sql = 'SELECT * FROM `' + stock + '_rt`' + 'WHERE date = \'' + date + '\' and time = \'' + time + '\'';
    connection.connect();
    connection.query(sql, function (err, result) {
        if (err) {
...
function get_lastmin(stock, date, time, price, volume) {
    ...
    var sql = 'SELECT * FROM `' + stock + '_rt`' + 'WHERE date = \'' + date + '\' and time = \'' + last_time + '\'';
    connection.connect();
    connection.query(sql, function (err, result) {
        if (err) {
...
function update_close() {
    ...
    var sql = 'select table_name from information_schema.tables where table_schema = \'' + con_db + '\'';
    connection.connect();
    connection.query(sql, function (err, result) {
        if (err) {
